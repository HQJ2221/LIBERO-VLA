...
class PolicyDecorator(nn.Module):
    """turn skill hyper to action delta / condition vector"""
    def __init__(self, cfg, shape_meta):
        super().__init__()
        self.hyper_dim = cfg.policy.hyper_dim
        self.output_dim = shape_meta['ac_dim']  # 与 base latent 对齐
        self.mlp = nn.Sequential(
            nn.Linear(self.hyper_dim, 256),
            nn.ReLU(),
            nn.Linear(256, self.output_dim),
        )

    def forward(self, hyper_param):
        return self.mlp(hyper_param)  # (B, embed_size)
...
class TransformerBasePolicy(BasePolicy):
    """
    Input: (o_{t-H}, ... , o_t)
...
class ActionExpert(BasePolicy):
    """一个 cluster 对应一个 ActionExpert"""
    def __init__(self, cfg, shape_meta):
        super().__init__(cfg, shape_meta)

        self.base_policy = TransformerBasePolicy(cfg, shape_meta)
        self.decorator = PolicyDecorator(cfg, shape_meta)
        self.cluster_id = None   # set in builder

    def forward(self, data):
        # base 输出 distribution（保持原有）
        base_dist = self.base_policy.forward(data)  # 返回 policy_head 的 dist
        
        # decorator
        hyper = data['skill_hyper_param']
        delta = self.decorator(hyper)  # (B, embed_size)

        if isinstance(base_dist, torch.distributions.MixtureSameFamily):
            # Extract internals for GMM
            means = base_dist.component_distribution.base_dist.loc  # [B, num_mixtures, action_dim]
            scales = base_dist.component_distribution.base_dist.scale  # [B, num_mixtures, action_dim]
            logits = base_dist.mixture_distribution.logits  # [B, num_mixtures]

            # Add delta to means (assume delta [B, action_dim])
            delta_expanded = delta.unsqueeze(1)  # [B, 1, action_dim]
            new_means = means + delta_expanded

            # Rebuild
            new_normal = D.Normal(loc=new_means, scale=scales)
            new_compo = D.Independent(new_normal, 1)
            new_mix = D.Categorical(logits=logits)
            new_dist = D.MixtureSameFamily(mixture_distribution=new_mix, component_distribution=new_compo)
        elif isinstance(base_dist, DiffusionDist):
            pred_action = base_dist.sample()
            print(pred_action.shape)
            ac_dim = pred_action.shape[-1]
            mean = pred_action + delta[:, :ac_dim]
            return mean     # FIXME: impossible for diffusion to return a dist?
        else:
            # Fallback for simple distributions (e.g., Normal)
            ac_dim = base_dist.mean.shape[-1]
            mean = base_dist.mean + delta[:, :ac_dim]
            new_dist = type(base_dist)(loc=mean, scale=base_dist.scale)

        return new_dist
    
    def get_action(self, data):
        # 确保 cluster_id 一致（单 expert 场景）
        if "cluster_id" in data:
            data["cluster_id"] = torch.tensor(self.cluster_id, dtype=torch.long, device=data["cluster_id"].device)
        return super().get_action(data)   # 或直接实现 sample
